...
outreg2 using "$output/RECAR_tabla1.doc", nocons ctitle("Peso Neto (Tn)") dec(2) label nonotes addnote("Errores estandar entre parentesis", "*** p<0.01, ** p<0.05, * p<0.1", "El efecto promedio total de la politica sobre el monto FOB en el primer mes tratado es negativo, con un valor de -2.37") replace

reg MontoFOBenmill treat t treat_t, robust
display (1.534542-.0333547*117) // El efecto total promedio de la política para el primer mes tratado ya arroja un valor negativo de -2.3679579. Como el efecto total de la intervención es decreciente a medida que pasa el tiempo, el efecto total pormedio sobre el monto FOB para todos los meses tratados será negativo.
outreg2 using "$output/RECAR_tabla1.doc", nocons ctitle("Monto FOB (Millones de USD)") dec(2) label nonotes addnote("Errores estandar entre parentesis", "*** p<0.01, ** p<0.05, * p<0.1", "El efecto promedio total de la politica sobre el monto FOB en el primer mes tratado es negativo, con un valor de -2.37") append 
...
newey MontoFOBenmill treat t treat_t, lag(4)
