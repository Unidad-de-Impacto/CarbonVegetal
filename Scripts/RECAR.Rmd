...
Cargo el dataset
```{r}
carbon <- read_excel("/Users/ariquelme/Downloads/Exportaciones de Carbon vegetal stata.xlsx", sheet = "Datos Anuales")
...
ton_año <- ggplot(carbon, aes(año, ton,
                    fill=factor(ifelse(año =="2012","Highlighted","Normal")))) + #le asigno una categoria a 2012 para diferenciarlo del resto de los años y poder colorearlo posteriormente
  geom_bar(stat = "identity") +
  geom_smooth(aes(group = año >= cutoff), #armo la linea de tendencia, utilizando una regresión lineal simple 
              method = "lm",
              color = "#E6B861",
              alpha = 0.1) +
  annotate("text", x = 2015, #agrego un texto y sombreado indicando cuando se implemento el RECAR
  y = 100000, 
  label = "Implementación del 
  RECAR",
  family = "montserrat",
  size = 7) +
  annotate("rect",xmin = 2012, xmax = 2018, ymin = 87000, ymax = 110000,
  alpha = .1) +
  ...
  labs(title = "Exportaciones de carbón vegetal (toneladas) ", #agrego el titulo
      caption = "Datos hasta junio del 2024
...
usd_año <- ggplot(carbon, aes(año, miles_usd,
  ...
  family = "montserrat",
  size = 7) +
  annotate("rect",xmin = 2012, xmax = 2018, ymin = 30000, ymax = 40000,
  alpha = .1) +
  ...
  theme(legend.position = "none",
        ...
        text = element_text(size = 14,family = "montserrat")) + 
  scale_x_continuous(breaks = seq(2002, 2024, by = 2)) +
  labs(title = "Exportaciones de carbón vegetal (miles USD)",
      caption = "Datos hasta junio del 2024
      Fuente: Elaboración propia en base a datos del INDEC")+
...
```{r}
base <- read_excel("/Users/ariquelme/Downloads/Exportaciones de Carbon vegetal stata.xlsx", sheet = "Datos Mensuales")


base$`en usd` <- base$`en usd`/1000
base$year <- year(base$Fecha)
base <- base %>%
  dplyr::group_by(year) %>%
  dplyr::summarise(
    Cantidad_anual_Real = sum(`en usd`, na.rm = TRUE),
    .groups = "drop"
  )
```
...
before_2012 <- base %>% filter(year < cutoff)
after_2012 <- base %>% filter(year >= cutoff)
...
base <- read_excel("/Users/ariquelme/Downloads/Exportaciones de Carbon vegetal stata.xlsx", sheet = "Datos Mensuales")
base$`en kg` <- base$`en kg`/1000
base$year <- year(base$Fecha)
base <- base %>%
  dplyr::group_by(year) %>%
  dplyr::summarise(
    Cantidad_anual_Real = sum(`en kg`, na.rm = TRUE),
    .groups = "drop"
  )
```
